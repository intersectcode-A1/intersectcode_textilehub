    ...
    <ol class="list-none p-0 inline-flex items-center">
        @foreach($items as $item)
            <li class="flex items-center">
                @if(isset($item['link']))
                    <a href="{{ $item['link'] }}" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">{{ $item['text'] }}</a>
                @else
                    <span class="text-gray-400 dark:text-gray-500">{{ $item['text'] }}</span>
                @endif
