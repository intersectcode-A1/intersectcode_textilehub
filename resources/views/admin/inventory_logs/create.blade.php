...
@section('title', 'Tambah Log Inventory')
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Tambah Log Inventory</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Catat pergerakan stok produk</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <form method="POST" action="{{ route('inventory-logs.store') }}">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="flex items-center p-4 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                            <input type="radio" name="type" value="in" {{ old('type') == 'in' ? 'checked' : '' }} class="w-4 h-4 text-green-600 bg-gray-100 border-gray-300 focus:ring-green-500 dark:focus:ring-green-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                            <div class="ml-3">
                        ...
                        <label class="flex items-center p-4 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                            <input type="radio" name="type" value="out" {{ old('type') == 'out' ? 'checked' : '' }} class="w-4 h-4 text-red-600 bg-gray-100 border-gray-300 focus:ring-red-500 dark:focus:ring-red-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                            <div class="ml-3">
                                <div class="flex items-center">
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="quantity" id="quantity" min="1" value="{{ old('quantity') }}" required class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('quantity') border-red-500 @enderror" placeholder="Masukkan jumlah">
                    @error('quantity')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror" placeholder="Contoh: Pembelian dari supplier, Penjualan, Penyesuaian stok, dll.">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="reference_type" id="reference_type" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('reference_type') border-red-500 @enderror">
                        ...
                        <option value="purchase" {{ old('reference_type') == 'purchase' ? 'selected' : '' }}>Pembelian</option>
                        <option value="order" {{ old('reference_type') == 'order' ? 'selected' : '' }}>Penjualan</option>
                        <option value="adjustment" {{ old('reference_type') == 'adjustment' ? 'selected' : '' }}>Penyesuaian Stok</option>
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="reference_id" id="reference_id" min="1" value="{{ old('reference_id') }}" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('reference_id') border-red-500 @enderror" placeholder="Contoh: ID Order, ID Pembelian, dll.">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Update quantity max based on selected product and type
    function updateQuantityMax() {
        const selectedProduct = productSelect.options[productSelect.selectedIndex];
        const selectedType = document.querySelector('input[name="type"]:checked')?.value;
        
        if (selectedProduct && selectedType === 'out') {
            const stockText = selectedProduct.text.match(/Stok: (\d+)/);
            if (stockText) {
    ...
    typeInputs.forEach(input => input.addEventListener('change', updateQuantityMax));
});
</script>
@endpush
@endsection 