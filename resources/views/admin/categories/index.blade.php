...
<div class="bg-gradient-to-r from-blue-400 to-blue-200 dark:from-blue-600 dark:to-blue-800 rounded-t-xl shadow-lg overflow-hidden">
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-blue-200 dark:divide-blue-700 text-xs sm:text-sm">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-blue-100 dark:divide-blue-700">
                @forelse ($categories as $index => $category)
                    <tr class="hover:bg-blue-50 dark:hover:bg-blue-900/50 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-block px-3 py-1 rounded-full bg-gradient-to-r from-blue-400 to-blue-300 dark:from-blue-500 dark:to-blue-400 text-white font-bold shadow">{{ $index + 1 }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <div class="flex flex-row items-center justify-end space-x-2">
                                <a href="{{ route('categories.edit', $category->id) }}" class="p-2 bg-blue-100 hover:bg-blue-400 dark:bg-blue-900 dark:hover:bg-blue-600 text-blue-600 hover:text-white dark:text-blue-300 dark:hover:text-white rounded-full shadow transition-all duration-200">
                                ...
                                <form action="{{ route('categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 bg-red-100 hover:bg-red-400 dark:bg-red-900 dark:hover:bg-red-600 text-red-600 hover:text-white dark:text-red-300 dark:hover:text-white rounded-full shadow transition-all duration-200">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr>
                        <td colspan="3" class="text-center py-8 sm:py-12">
                            <div class="text-gray-500 dark:text-gray-400">
                                <svg class="mx-auto h-8 w-8 sm:h-12 sm:w-12 text-gray-400 dark:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                                ...
                                <p class="mt-1 text-xs sm:text-sm text-gray-500 dark:text-gray-400">Mulai dengan menambahkan kategori baru.</p>
