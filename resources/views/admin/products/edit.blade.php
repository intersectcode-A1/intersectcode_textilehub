...
<div class="p-6 max-w-lg mx-auto">
    <h1 class="text-2xl font-bold mb-4 text-white">Edit Produk</h1>

    <form action="{{ route('products.update', $data->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label for="nama" class="block text-white font-semibold">Nama Produk</label>
            <input type="text" name="nama" id="nama" value="{{ old('nama', $data->nama) }}" class="w-full border border-gray-300 rounded px-3 py-2 text-black bg-white" required>
            @error('nama')
        ...
        <div>
            ...
            <input type="number" name="harga" id="harga" value="{{ old('harga', $data->harga) }}" class="w-full border border-gray-300 rounded px-3 py-2 text-black bg-white" required>
            @error('harga')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" name="stok" id="stok" value="{{ old('stok', $data->stok) }}" class="w-full border border-gray-300 rounded px-3 py-2 text-black bg-white" required>
            @error('stok')
                <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            @if($data->foto)
                <img src="{{ asset('storage/' . $data->foto) }}" alt="{{ $data->nama }}" width="150" class="mt-2 rounded">
            @endif
        </div>
