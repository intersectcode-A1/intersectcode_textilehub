<div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-2 sm:gap-0">
        <h3 class="text-base sm:text-lg font-semibold">Daftar Varian</h3>
        <a href="{{ route('admin.variants.create', $product->id) }}" 
           class="inline-flex items-center px-3 sm:px-4 py-2 border border-transparent rounded-md shadow-sm text-xs sm:text-sm font-medium text-white bg-primary-600 hover:bg-primary-700">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-xs sm:text-sm">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($product->variants as $variant)
                        <tr>
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                {{ $variant->name }}
                            </td>
                            ...
                            <td class="px-4 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex flex-col sm:flex-row justify-end space-y-1 sm:space-y-0 sm:space-x-2">
                                    <a href="{{ route('admin.variants.edit', $variant->id) }}" 
                                       class="text-primary-600 hover:text-primary-900 text-xs sm:text-sm">Edit</a>
