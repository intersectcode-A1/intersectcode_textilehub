...
<div class="mb-4 sm:mb-6">
    <h1 class="text-2xl font-extrabold text-blue-900 dark:text-blue-200 mb-2">Edit Supplier</h1>
    ...
    <form action="{{ route('supplier.update', $supplier->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="alamat" id="alamat" value="{{ old('alamat', $supplier->alamat) }}"
                class="w-full px-4 py-3 rounded-xl bg-white/60 dark:bg-gray-900 border-2 border-blue-100 dark:border-gray-700 text-blue-900 dark:text-blue-100 placeholder-blue-300 dark:placeholder-blue-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50 transition-all duration-200 shadow-sm">
            @error('alamat')<p class="mt-2 text-sm text-red-500">{{ $message }}</p>@enderror
        </div>
        <div>
            ...
            @error('kontak')<p class="mt-2 text-sm text-red-500">{{ $message }}</p>@enderror
        </div>
        <div>
            <label for="produk" class="block text-base font-bold text-blue-900 dark:text-blue-200 mb-1">Produk yang Disediakan</label>
            <input type="text" name="produk" id="produk" value="{{ old('produk', $supplier->produk) }}"
                class="w-full px-4 py-3 rounded-xl bg-white/60 dark:bg-gray-900 border-2 border-blue-100 dark:border-gray-700 text-blue-900 dark:text-blue-100 placeholder-blue-300 dark:placeholder-blue-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50 transition-all duration-200 shadow-sm">
            @error('produk')<p class="mt-2 text-sm text-red-500">{{ $message }}</p>@enderror
        </div>
        <div class="mb-4">
            <label for="deskripsi" class="block text-base font-bold text-blue-900 dark:text-blue-200 mb-1">Deskripsi</label>
            <textarea name="deskripsi" id="deskripsi" rows="3"
                class="w-full px-4 py-3 rounded-xl bg-white/60 dark:bg-gray-900 border-2 border-blue-100 dark:border-gray-700 text-blue-900 dark:text-blue-100 placeholder-blue-300 dark:placeholder-blue-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50 transition-all duration-200 shadow-sm"
                placeholder="Masukkan deskripsi supplier">{{ old('deskripsi', $supplier->deskripsi) }}</textarea>
        </div>
