...
<div class="mb-6">
    ...
    <a href="{{ route('orders.index') }}" class="inline-flex items-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-all duration-200">
        <svg class="w-5 h-5 mr-2 -ml-1" fill="none" stroke="currentColor" viewBy="0 0 24 24">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($order->items as $item)
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-6 py-4">
                                    @php
                                        $variants = $item->variant_info;
                                        if (is_array($variants) && isset($variants['type'])) {
                                            $variants = [$variants]; // jika hanya satu varian, bungkus jadi array
                                        }
                                    @endphp
                                    @if(!empty($variants) && is_array($variants))
                                        @foreach($variants as $variant)
                                            <div>
                                                <span class="font-semibold">{{ ucfirst($variant['type'] ?? '') }}</span>: {{ $variant['name'] ?? '' }}
